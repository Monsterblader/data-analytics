...
path = '/Users/paulbacchus/Work/mckinsey_gp/'

wb = xlrd.open_workbook(path + 'data.xlsx')
wb = xlrd.open_workbook(path + 'datav2.xlsx')
...
with open(path + 'data-analytics/data/operations_costs.json', 'w') as f:
...
for i in range(20, 34):
    headcount_rows[i] = headcount_rows[i][0:3]

for i in range(21, 32):
    headcount_rows[i][0] = xlrd.xldate_as_tuple(headcount_rows[i][0], 0)

headcount_d = {
    'BU_Breakdown_YTD':
    {'headers': headcount_rows[0], 'values': headcount_rows[1:9], 'footer': headcount_rows[-1]},
    'BU_Breakdown_Monthly':
    {'headers': headcount_rows[10], 'values': headcount_rows[11:19], 'footer': headcount_rows[-1] },
    'Total_FTEs':
    {'headers': headcount_rows[20], 'values': headcount_rows[21:]},
}

with open(path + 'data-analytics/data/headcount.json', 'w') as f:
...
capacity_d = {
    'Availability':
    {'headers': capacity_rows[0][0:9], 'values': cap_avail},
    'Utilization':
    {'headers': capacity_rows[0][11:], 'values': cap_util}
}

with open(path + 'data-analytics/data/Capacity.json', 'w') as f:
...
with open(path + 'data-analytics/data/Colleagues.json', 'w') as f:
